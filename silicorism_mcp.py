...
INSTRUCTIONS = (
    ...
    "3. MASTER PLAN: if the superpowers skills are available to you, run "
    "superpowers:brainstorming to settle the design with the user, then "
    "superpowers:writing-plans to turn it into numbered tasks — those tasks are "
    ...
    "the only way the pipeline can tell. Between the red gate and the "
    "implementer, put a glm-5 review node that reads the tests against the "
    "spec and reports contradictions: expect_fail proves the tests are red, "
    "not that they are right.\n"
    "`writes` is a fence, not a note: the worker fails a node that modified a "
    "file it did not claim, so a builder cannot rewrite the tests it is "
    "measured by. Give every building node its file list. And anything two "
    "nodes both touch — a constructor signature, an interface, a file layout — "
    "must be spelled out in BOTH prompts, or the second node implements to a "
    "contract the first one invented.\n\nYOU are the orchestrator: you ask the questions, write the plan and gate the "
